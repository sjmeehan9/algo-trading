"""ML training session orchestration with path management and audit logging."""
...
class TrainML:
    """Orchestrates ML training sessions, path setup, and audit recording."""
    ...
    def _path_setup(self) -> dict:
        ...
        self.input_filepath = os.path.join(
            pipeline_data_path, self.input_filename + model_file_ext
        )
        ...
        if self.evaluate:
            ...
            self.eval_filepath = os.path.join(
                pipeline_data_path, self.eval_filename + model_file_ext
            )
            ...
            if os.path.exists(self.eval_filepath):
                ...
                raise FileNotFoundError(
                    f"Input model not found at: {self.eval_filepath}"
                )
        else:
            if os.path.exists(self.model_filepath + model_file_ext):
                # Ask user if they want to overwrite the file
                overwrite = input(
                    f"{self.model_filename} already exists. Do you want to overwrite it? (y/n) "
                )
                if overwrite == "n":
    ...
    def session_info(self) -> None:
        session = {"pipeline": self.pipeline_name, "model": self.model_filename}
    ...
    def training_factory(self, pipeline_type: str) -> object:
        if pipeline_type == "rl":
            ...
            raise ValueError("Pipeline type not recognised")

    def start(self) -> None:
        # Instanciate training object
        self.training = self.training_factory(
            self.pipeline["pipeline"]["pipeline_type"]
        )
